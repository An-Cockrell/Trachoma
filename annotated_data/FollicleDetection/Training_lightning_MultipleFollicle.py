...
class TrachomaGradableArea(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        outputs = self.model(images)["out"]  # <- no .squeeze()
        if targets.dim() == 3:  # (B, H, W) -> (B, 1, H, W)
            targets = targets.unsqueeze(1)
        targets = targets.to(dtype=outputs.dtype, device=outputs.device)
...
class DiceLoss(nn.Module):
    ...
    def forward(self, y_pr, y_gt):
        # print(f"y_pr shape: {np.shape(y_pr)}")
        # print(f"y_gt shape: {np.shape(y_gt)}")
        return 1 - f_score(
            y_pr,
            y_gt,
            beta=1.0,
...
def run_experiment(
    ...
    dataloader,
    model,
    project="Trachoma_GradeableArea",
    accum_batches=1,
    swa=False,
):
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=f"Checkpoints/{run_info}/run_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
        save_last=True,
        save_top_k=1,
        mode="min",
        monitor="val_loss",
...
if __name__ == "__main__":
    ...
    fcn = models.segmentation.fcn_resnet50(weights=None, num_classes=1)
    print(fcn)
    segModel = TrachomaGradableArea(fcn)
    # CCM means Chris, Clara, Manny, and def_follicles means we didn't include possible follicles
    run_info = "MultipleFollicle_all_data_cropped_images"
