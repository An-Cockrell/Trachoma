...
class TrachomaClassifier(pl.LightningModule):
    def __init__(self, model, optimizer_metric="val_loss", weight=None, threshold=0.5):
        ...
        # metrics
        self.train_acc = torchmetrics.Accuracy(task="binary", threshold=threshold)
        self.val_acc = torchmetrics.Accuracy(task="binary", threshold=threshold)
        self.test_acc = torchmetrics.Accuracy(task="binary", threshold=threshold)

        self.train_precision = torchmetrics.Precision(
            task="binary", threshold=threshold
        )

        self.val_precision = torchmetrics.Precision(task="binary", threshold=threshold)
        self.test_precision = torchmetrics.Precision(task="binary", threshold=threshold)

        self.train_recall = torchmetrics.Recall(task="binary", threshold=threshold)
        self.val_recall = torchmetrics.Recall(task="binary", threshold=threshold)
        self.test_recall = torchmetrics.Recall(task="binary", threshold=threshold)

        self.train_f1 = torchmetrics.F1Score(task="binary", threshold=threshold)
        self.val_f1 = torchmetrics.F1Score(task="binary", threshold=threshold)
        self.test_f1 = torchmetrics.F1Score(task="binary", threshold=threshold)
    ...
    def configure_optimizers(self):
        ...
        return {
            "optimizer": optimizer,
            "lr_scheduler": {
                "scheduler": ReduceLROnPlateau(optimizer, mode=mode, patience=3),
                "interval": "epoch",
                "frequency": 1,
...
def run_experiment(
    ...
    # train
    wandb_logger = WandbLogger(project=project, entity=entity, name=run_info)
    early_stop_callback = EarlyStopping(
        monitor="val_loss", min_delta=0.00, patience=5, verbose=True, mode="min"
    )
    checkpoint_callback = ModelCheckpoint(
        dirpath="Checkpoints/{}".format(run_info),
        save_last=True,
        save_top_k=1,
        mode="min",
        monitor="val_loss",
...
if __name__ == "__main__":
    ...
    run_info = f"all_clean_data/run_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    run_experiment(run_info, dm, classifier12)
