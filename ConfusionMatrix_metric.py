...
path = '/media/dsocia22/T7/Trachoma/TrainedModels/Pytorch_lightning_consensus_oversample5_posweight4_follicleenhance_flip_rotate_norm_pretrained_accum5batch_resnet101/epoch=8-step=323.ckpt'
# path = '/media/dsocia22/T7/Trachoma/TrainedModels/Pytorch_lightning_consensus_oversample5_flop_rotate_perspective_pretrained_norm_resnet101/epoch=11-step=2159.ckpt'
thresh = 0.05
print(path, thresh)
...
for batch, t in zip(test_data, test_true):
    ...
    for i, l in enumerate(L):
        r.loc[len(r.index)] = [batch['name'][i], int(batch['label'][i]), 1 if pred[i] > thresh else 0]
        if l == 1:
            if pred[i] < thresh:
...
r.to_csv('BestModelResults_NewThresh.csv')
#
